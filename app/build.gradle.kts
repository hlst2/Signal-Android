...
android {
  ...
  defaultConfig {
    ...
    // Sealed-sender (unidentified delivery) certs are verified against these trust-root public keys. A
    // self-hosted server signs its sender certs with its OWN unidentifiedDelivery key, so set
    // PRIVATE_UNIDENTIFIED_SENDER_TRUST_ROOTS (base64 EC public key) to that server key or sealed-sender
    // messages will be rejected. The Signal production roots below are only a fallback.
    buildConfigField("String[]", "UNIDENTIFIED_SENDER_TRUST_ROOTS", (project.findProperty("PRIVATE_UNIDENTIFIED_SENDER_TRUST_ROOTS") as String?)?.takeIf { it.isNotBlank() }?.let { "new String[]{ \"$it\" }" } ?: "new String[]{ \"BXu6QIKVz5MA8gstzfOgRQGqyLqOwNKHL6INkv3IHWMF\", \"BUkY0I+9+oPgDCn4+Ac6Iu813yvqkDr/ga8DzLxFxuk6\"}")
    // Private/self-hosted deployments MUST replace these with the public params derived from the server's own
    // zkgroup secret (run `java -jar server.jar zkparams` on the server). Override per-build without editing code
    // by setting PRIVATE_ZKGROUP/GENERIC/BACKUP_SERVER_PUBLIC_PARAMS in gradle.properties or via -P. The Signal
    // production values below are only a fallback and will NOT verify against a self-hosted server.
    buildConfigField("String", "ZKGROUP_SERVER_PUBLIC_PARAMS", "\"${(project.findProperty("PRIVATE_ZKGROUP_SERVER_PUBLIC_PARAMS") as String?)?.takeIf { it.isNotBlank() } ?: ""}\"")
    buildConfigField("String", "GENERIC_SERVER_PUBLIC_PARAMS", "\"${(project.findProperty("PRIVATE_GENERIC_SERVER_PUBLIC_PARAMS") as String?)?.takeIf { it.isNotBlank() } ?: ""}\"")
    buildConfigField("String", "BACKUP_SERVER_PUBLIC_PARAMS", "\"${(project.findProperty("PRIVATE_BACKUP_SERVER_PUBLIC_PARAMS") as String?)?.takeIf { it.isNotBlank() } ?: ""}\"")
